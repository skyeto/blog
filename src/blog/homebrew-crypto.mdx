...
Before diving into the specifics though, an important aside on what <abbr class="smallcaps">(V)OPRF</abbr>'s
are and what they accomplish in a Privacy Pass scheme.
...
verify that their token doesn't contain any metadata that are attributable to
them, and that the server isn't underhandedly generating unique keys each time
it's issuing new tokens.

Since making the tokens verifiable is an extension, I'll begin with <abbr class="smallcaps">OPRF</abbr>'s:

### Oblivious Pseudorandom Functions

The first goal is to create a token that the server can issue and verify,
without learning the message until it verifies the token. We do this by
introducing the concept of a blind.
...
* raphaelrobert on [Github](https://github.com/raphaelrobert) for
  implementing the <abbr class="smallcaps">VOPRF</abbr> crypto primitives in Rust, used both on the client and
  server. (MIT)
* FriendlyCaptcha on
...
[^kagi]: There's an inherent risk of linkability in the
    ...
    a most things, Privacy Passes aren't a panacea, but in practice Kagi
